...
// set font size
func (e *Escpos) SetFontSize(width, height uint8) error {
...
// set alignment
func (e *Escpos) SetAlign(align string) error {
	a := 0
	switch align {
	case "left":
		a = 0
	...
	case "right":
		a = 2
	default:
		return fmt.Errorf("Invalid alignment: %s", align)
	}
...
func (e *Escpos) Text(params map[string]string, data string) {
	...
	if x, ok := params["x"]; ok {
		if i, err := strconv.Atoi(x); err == nil {
			...
			log.Fatalf("Invalid x param %v", x)
		}
	}
	...
	if y, ok := params["y"]; ok {
		if i, err := strconv.Atoi(y); err == nil {
			...
			log.Fatalf("Invalid y param %v", y)
		}
	}
...
func (e *Escpos) Feed(params map[string]string) {
	...
	if l, ok := params["line"]; ok {
		if i, err := strconv.Atoi(l); err == nil {
			...
			log.Fatal(fmt.Sprintf("Invalid line number %v", l))
		}
	}
	...
	if u, ok := params["unit"]; ok {
		if i, err := strconv.Atoi(u); err == nil {
			...
			log.Fatal(fmt.Sprintf("Invalid unit number %v", u))
		}
	}
...
func (e *Escpos) Image(params map[string]string, data string) {
	...
	if err != nil {
		log.Fatalf("Invalid image width %v", wstr)
	}
